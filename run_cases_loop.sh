...
declare -a num_nodes_cases=( 1000 ) # 50 100 400 800 ) #  1200 1500 2000
...
declare -a overlappingUNLs=( 0.37 0.38 0.39 0.40 0.41 0.42 0.43 0.44 0.45)

outputfile="./cases_output4-4.csv"
...
for nncase in "${num_nodes_cases[@]}"; do
	for pcmal in "${percentage_malicious[@]}";do
		for ovUNLs in "${overlappingUNLs[@]}"; do
			...
			$simulatorExe --num_nodes $nncase --mal_nodes_pC $pcmal  --overlappingUNLs $ovUNLs --max_e2c_latency 25 --min_e2c_latency 25 --max_c2c_latency 80 --min_c2c_latency 80 >> $outputfile 
		done
	done
done
